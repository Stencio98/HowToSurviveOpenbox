...
*Questo passaggio non è fondamentale, quindi sentiti libero di fare come preferisci,puoi creare un Makefile (il Makefile presente qui è quello che uso quotidianamente assieme al .bashrc e che tengo aggiornato) nella directory `/home/user/` in cui ho inserito i comandi che uso più spesso, una alternativa è editare il file `/home/user/.bashrc` (prima fanne una copia, per esempio `cp .bashrc .bashrc.backup`, nel caso tu abbia perso il file `cp /etc/skel/.bashrc /home/user/.bashrc`) per personalizzare comandi e poterli utilizzare ovunque ti trovi e non solo quando sei in `/home/user/`. 
