...
*Questo passaggio non è fondamentale, quindi sentiti libero di fare come preferisci, puoi creare un Makefile (il Makefile presente qui è quello che uso quotidianamente assieme al .bashrc e che tengo aggiornati) nella directory `/home/user/` in cui inserire i comandi usati più spesso. Il file `/home/user/.bashrc` (prima fanne una copia, per esempio `cp .bashrc .bashrc.backup`, nel caso venga perso il file è possibile recuperarlo così `cp /etc/skel/.bashrc /home/user/.bashrc`) permette di personalizzare comandi e poterli utilizzare ovunque ti trovi e non solo quando sei in nella directory `/home/user/`. 
